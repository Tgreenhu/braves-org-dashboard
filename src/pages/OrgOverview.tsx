      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="mb-3 flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            {record.gamesBack != null && (
              <span className="text-xs text-navy-900/45">GB {record.gamesBack}</span>
            )}
        ...
        {/* Next Game */}
        {record.nextGameOpponent && (
          <a
            href={record.nextGameUrl ?? undefined}
            target="_blank"
            rel="noreferrer"
            className="mb-3 flex items-center justify-between rounded-lg bg-navy-950/[0.03] px-2.5 py-1.5 text-xs hover:bg-navy-950/[0.06]"
          >
            <span className="text-navy-900/50">Next Game</span>
            <span className="font-medium text-navy-950">
              {record.nextGameDate ? `${record.nextGameDate} ` : ''}
              {record.nextGameOpponent}
            </span>
          </a>
        )}
        ...
        <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
          ...
          {record.xRecord && <MiniStat label="X-W/L" value={record.xRecord} />}
          {record.vs500Record && <MiniStat label="≥.500" value={record.vs500Record} />}
        </div>

        {/* Slash + pitching line — only shown once you've uploaded hitter/pitcher
            stats, since MLB.com/MiLB.com standings pages don't include these */}
        {(record.avg != null || record.era != null) && (
          <div className="mt-3 overflow-x-auto">
            <table className="stat-table">
              ...
              <tbody>
                <tr>
                  <td>Team</td>
                  <td>{record.avg != null ? record.avg.toFixed(3) : '—'}</td>
                  <td>{record.obp != null ? record.obp.toFixed(3) : '—'}</td>
                  <td>{record.slg != null ? record.slg.toFixed(3) : '—'}</td>
                  <td>{record.ops != null ? record.ops.toFixed(3) : '—'}</td>
                  <td>{record.era != null ? record.era.toFixed(2) : '—'}</td>
                  <td>{record.fip != null ? record.fip.toFixed(2) : '—'}</td>
                  <td>{record.siera != null ? record.siera.toFixed(2) : '—'}</td>
                </tr>
              </tbody>
            </table>
          </div>
